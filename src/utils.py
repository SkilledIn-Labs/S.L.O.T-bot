...
def save_token_to_DB(mobile, token):
    return True


def get_token_from_DB(mobile):
    return True


def save_user_info_to_DB(collected_details):
    return True
...
def collect_user_details(request_header):
    ...
    refresh_freq = input('How often do you want to refresh the calendar (in seconds)? Default 15. Minimum 3. : ')
    refresh_freq = int(refresh_freq) if refresh_freq and int(refresh_freq) >= 3 else 15
    ...
    auto_book = input("Do you want to enable auto-booking? (y or n) Default n: ")
...
def check_and_book(request_header, beneficiary_dtls, location_dtls, search_option, **kwargs):
    ...
    try:
        ...
        if len(tmp_options) > 0:
            ...
            if auto_book == 'y' or auto_book == 'yes':
                ...
                choice = inputimeout(
                    prompt='----------> Wait 20 seconds for updated options OR \n----------> Enter a choice e.g: 1.4 '
                           'for (1st center 4th slot): ',
                    timeout=20)
